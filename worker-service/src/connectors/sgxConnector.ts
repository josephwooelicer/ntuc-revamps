...
export class SgxConnector {
  ...
  constructor(source) {
    ...
    this.announcementsUrl =
      process.env.SGX_ANNOUNCEMENTS_URL || "https://www.sgx.com/securities/company-announcements";
  }

  async pull(range, cursor = null, options: { query?: string } = {}) {
    if (cursor) return { documents: [], nextCursor: null };
    const query = options.query || process.env.SGX_DEFAULT_QUERY || "restructuring";
    const url = `${this.announcementsUrl}?${new URLSearchParams({ keyword: query }).toString()}`;
    ...
    try {
      ...
      const err: any = new Error(`SGX browser scrape failed: ${error?.message || "unknown error"}`);
      err.status = 502;
      err.responseText = "";
